...
async function run() {
    try {
        ...
        console.log("Cleared all old notices.");
        ...
        const richAnnouncements = [
            ...
            },
            {
                ...
                targetAudience: 'All',
                visibleForHours: 336, // 14 days
                expiresAt: hoursFromNow(336),
            },
            {
                ...
                targetAudience: 'All',
                visibleForHours: null, // Permanent milestone announcement
                expiresAt: null,
            },
            {
                ...
                visibleForHours: 360, // 15 days deadline
                expiresAt: hoursFromNow(360),
            },
            {
                ...
                targetAudience: 'All',
                visibleForHours: 504, // 21 days
                expiresAt: hoursFromNow(504),
            },
            {
                ...
                targetAudience: 'All',
                visibleForHours: 240, // 10 days to register
            ...
            },
            {
                ...
                targetAudience: 'All',
                visibleForHours: 72, // 3 days — registration closes soon
        ...
        for (const compId of companyIds) {
            const author = admins.find(a => a._id.toString() === compId || a.company?.toString() === compId) || admins[0] || employees[0];
            for (const tpl of richAnnouncements) {
                const anc = new Announcement({
                    ...
                    visibleForHours: tpl.visibleForHours || null,
                    expiresAt: tpl.expiresAt || null,
                });
                await anc.save();
                totalInserted++;
            }
        }
        console.log(`✅ Seeded ${totalInserted} announcements with expiry dates across all companies!`);
        process.exit(0);
    } catch (e) {
